...
SVGSpriteCss.prototype._layout = function() {
    ...
    const pseudoShapeMap = new Map();
    for (const shape of this._spriter._shapes) {
        pseudoShapeMap.set(shape.base, pseudoShapeMap.get(shape.base) || Boolean(shape.state));
    }
    ...
    for (const [index, shape] of this.data.shapes.entries()) {
        ...
        if (this._displaceable || !shape.master) {
            ...
            if (!shape.master) {
                positionMap.set(this._spriter._shapes[index].id, _.pick(shape.position.absolute, ['x', 'y']));
            }
            ...
            if (svgX.length > 1) {
                x = shape.master ? shape.position.absolute.x - positionMap.get(shape.master).x : shape.position.absolute.x;
            ...
            if (svgY.length > 1) {
                y = shape.master ? shape.position.absolute.y - positionMap.get(shape.master).y : shape.position.absolute.y;
 ...
 *
 * @param {Map} pseudoShapeMap      Pseudo shape map
