...
SVGSpriteCssPacker.prototype.fit = function() {
    ...
    const width = blocksLength ? this.blocks[0].width : 0;
    const height = blocksLength ? this.blocks[0].height : 0;

    this.root.width = width;
    this.root.height = height;

    for (const b of this.blocks) {
        const node = this._findNode(this.root, b.width, b.height);
        const fit = node ? this._splitNode(node, b.width, b.height) : this._growNode(b.width, b.height);
        this.positions[b.index] = { x: fit.x, y: fit.y };
    }
